...
if($text == '') {
	...
	$check = $wpdb->get_var($wpdb->prepare("SELECT MAX(index_antwort) FROM {$wpdb->prefix}sts_answers WHERE ticket_id = %d",$id));
		if($check != NULL) {
			$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}sts_answers (ticket_id, index_antwort, user, antwort) VALUES (%d, %d, %s, %s)",
